...
class AskLLM():
    def __init__(self,localhost,file_path):
        ...
        self.columns = ['question','answer','assistant','LLManswer','expert']
    ...
    def ask_llm(self, question,conversationId, type,assistantId,requestId):
        ...
        body = {"content": question, "assistantId": assistantId, "conversationId": conversationId, "type": type}
        result = ""
        assistant = "基模回答"
        ...
        try:
            ...
            for line in response.iter_lines(decode_unicode=True):
                if line:
                    ...
                    if data_dict["type"] == 1 and data_dict["status"] == 1:
                        ...
                        assistants = data_dict['answer'].get("assistants")
                        if assistantId == 1:
                            assistant = "炼化知识专家"
                        elif assistantId == 2:
                            assistant = "油气知识专家"
            ...
            return result,assistants,conversationId,request_id

    def ask_zy(self,question,answer,assistantId):
        LLManswer, assistans, conversationId,request_id = self.ask_llm(question=question, conversationId="", type=3,assistantId="",requestId="")
        if assistantId in assistans:
            expert,assistans,conversationId,request_id=self.ask_llm(question=question,conversationId=conversationId,type=6,assistantId=assistantId,requestId=request_id)
        else:
            expert = ""

        item = [question,answer,assistantId, LLManswer, expert]
        print(item)
...
if __name__ == '__main__':
    a = AskLLM(localhost="https://test-expert.ai.cnpc", file_path='D:/test1116.xlsx')
    # b = a.ask_llm(question="可再生能源系统需要什么样的蓄电池?", conversationId="", type=3)
    # # a.select_sql(request_id='a22b79c6-9f77-41a2-871f-53df3c081bb8',assistantId='')
    a.ask_zy(question="可再生能源系统需要什么样的蓄电池?",assistantId=5)
