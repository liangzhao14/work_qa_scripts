...
localhost = "https://assistant.ai.cnpc"
file_path = fr'D:\项目\中石油\PythonProject\ZYCJ\output\chat_{max_workers}路_{start_time}.xlsx'
df = pandas.read_excel(r'D:\项目\中石油\PythonProject\ZYCJ\input\questions_badcases.xlsx')
...
for m in range(0,len(questions),20):
    ...
    futures = [thread_pool.submit(AskLLM.ask_llm, question=batch[i], conversationId="") for i in range(len(batch))]
    try:
