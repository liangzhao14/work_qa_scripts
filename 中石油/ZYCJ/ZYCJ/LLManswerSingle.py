...
class AskLLM():
    def __init__(self,localhost,file_path):
        ...
        self.login("zhaoliang","Lui9UkQvnQdZVZtT6xTjmTP0y2Jk7/1/bm3Kfhpw/vM=")
    ...
    def ask_llm(self, question, conversationId):
        ...
        url = f"{self.localhost}/api/maintain/core/conversation/streamChat"
        ...
        try:
            ...
            end_time = start_time
            response = self.session.post(url=url, json=data, headers=headers, stream=True, verify=False, timeout=600)
                ...
                if sub_string in line:
                    ...
                    if data_dict["type"] == 1 and data_dict["status"] == 1:
                        if data_dict["target"] == "end":
                            ...
                            # break
                        elif data_dict["target"] == "update":
                            # print(data_dict["messages"][0]["text"])
                            answer_result.append(data_dict["messages"][0]["text"])
...
if __name__ == '__main__':
    ...
    a = AskLLM(localhost="https://assistant.ai.cnpc", file_path=fr'D:/ZSY/1128/output/test1114.xlsx')
    # a.select_sql(request_id='9ffd5fba-9330-4057-b4a6-5de064b97e9d')
    a.ask_llm(question="你好", conversationId="")
    # a.create_excel()