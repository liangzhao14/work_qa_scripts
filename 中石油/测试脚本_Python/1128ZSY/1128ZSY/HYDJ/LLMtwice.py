...
class AskLLM():
    def __init__(self,localhost,file_path):
        ...
        self.columns = ['first_question', 'second_question', 'rewrite']
        ...
        try:
            with connection.cursor() as cursor:
                ...
                sql = f"SELECT content FROM artisan_chat_history where " \
                          f"conversation_id = '{request_id}' and role = 'rewrite' limit 1"  # 替换为你的实际查询语句
                # print(sql)
    ...
    def ask_llm_twice(self,first_question,second_question):
        ...
        rewrite = self.select_sql(request_id=conversationId, assistantId="")
        item = [first_question, second_question, rewrite]
        print(item)
